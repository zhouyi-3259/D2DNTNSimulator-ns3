 ...
 * \ingroup leo
 *
 * Declaration of SatelliteMobilityModel
 */
...
public:
  ...
  /// destructor
  virtual ~SatelliteMobilityModel ();
   ...
   * @return
   */
  double GetRaan(const Vector &position);
  
  /**
   * @return
   */
  double GetTrueAnomaly(const Vector &position);

  Vector GetPositionAtTime(Time futureTime) const;
...
private:
  ...
  // double CalcLatitude () const;
    ...
    /**
   * \brief Calc the longitude depending on simulation time inside ITRF coordinate
   * system
   *
   * \return longitude
   */
